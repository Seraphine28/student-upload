...
export async function advisorApprovePortfolio(id) {
  const res = await fetch(`${BASE}/api/portfolioadmin/${id}/approve`, {
    method: "PUT",
    headers: { ...authHeader() },
  });
...
export async function advisorRejectPortfolio(id, feedback) {
  const res = await fetch(`${BASE}/api/portfolio/${id}/reject`, {
    method: "PUT",
    headers: {
...
export async function getInProcessPortfolios() {
  // ⚠️ backend หนูสะกด inProcress (มี R เกิน)
  const res = await fetch(`${BASE}/api/portfolio/inProcress`, {
    headers: { ...authHeader() },
  });
...
export async function superApprovePortfolio(id) {
  const res = await fetch(`${BASE}/api/portfoliosuper/${id}/approve`, {
    method: "PUT",
    headers: { ...authHeader() },
  });
...
export async function getUserAdminView(id) {
  const res = await fetch(`${BASE}/api/user/admin-view/${id}`, {
    headers: { ...authHeader() },
  });
